...
if selectedPromtOption == "Я напишу самостоятельно!":
    st.write(":red[Внимание!] От написание промта зависит качество суммаризации, если вы не уверены в своих силах - используйте готовые решения")
    promt = st.text_input("Введите свой промт-запрос")
else:
    promtRadio = st.radio("Выберите ваш промт-запрос",
                          ["Объясни какую задачу должен выполнить каждый участник этой встречи",
                           "Распиши задачи для каждого участника этой встречи",
                           "Вкратце расскажи о чем эта запись"])

    if promtRadio == "Объясни какую задачу должен выполнить каждый участник этой встречи":
        st.write("Вы выбрали: 'Объясни какую задачу должен выполнить каждый участник этой встречи'")
        promt = "Объясни какую задачу должен выполнить каждый участник этой реальной встречи:"

    if(promtRadio == "Распиши задачи для каждого участника этой встречи"):
        st.write("Вы выбрали: 'Распиши задачи для каждого участника этой встречи:'")
        promt = "Распиши задачи для каждого участника этой реальной встречи:"

    if(promtRadio == "Вкратце расскажи о чем эта запись"):
        st.write("Вы выбрали: 'Вкратце расскажи о чем эта запись:'")
        promt = "В крации расскажи о чем этот текст:"

    # можно добавить тег captions - он позволяет добавить надпись ниже radio button
